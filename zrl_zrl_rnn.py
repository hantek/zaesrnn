...
class SRNN(Model):
    def __init__(self, name,  # a string for identifying model.
        ...
        var = theano.shared(self.numpy_rng.uniform(
            low=-numpy.sqrt(3./self.numrl),
            high=numpy.sqrt(3./self.numrl),
            size=(self.numsz, self.numrl)).astype(theano.config.floatX)
        )
        ...
        var.set_value(self.numpy_rng.uniform(
            low=-numpy.sqrt(3./self.numrl),
            high=numpy.sqrt(3./self.numrl),
            size=(self.numsz, self.numrl)).astype(theano.config.floatX)
        )
        ...
        self.whl = [theano.shared(value=var0, name='whl0'),
                    theano.shared(value=var1, name='whl1')]
        self.wlh = [theano.shared(value=var0.T, name='wlh0'),
        ...
        self.hids_t1.append(ReLU(
            T.dot(T.dot(self.hids_t0[1], self.whl[1]), self.wlh[1]
            ) + T.dot(self.hids_t1[-1], self.wxh[1])
        ))
        self.x_pred_1 = self.bx + T.dot(self.hids_t1[0], self.whx[0]) + T.dot(self.hids_t1[1], self.whx[1])
        # end of one-step prediction

        def step(x_tm1, hids_tm1):
            hids_tm1 = [hids_tm1[:,                        : self.numsz              ], 
                        hids_tm1[:,  self.numsz            : self.numsz*2            ]]
            hids_t = [ReLU(
                T.dot(T.dot(hids_tm1[0], self.whl[0]), self.wlh[0]
                ) + T.dot(x_tm1, self.wxh[0])
            )]
            hids_t.append(ReLU(
                T.dot(T.dot(hids_tm1[1], self.whl[1]), self.wlh[1]
                ) + T.dot(hids_t[-1], self.wxh[1])
            ))
            x_pred_t = self.bx + T.dot(hids_t[0], self.whx[0]) + T.dot(hids_t[1], self.whx[1])
    ...
    def sample(self, numcases=1, numframes=10, temperature=1.0):
        ...
        preds[:, 0, :] = self.numpy_rng.multinomial(numcases, pvals=numpy.ones(self.numvis)/numpy.float(self.numvis))
