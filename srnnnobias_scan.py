...
class SRNN(Model):
    ...
    def __init__(self, name, numvis, numhid, numframes, output_type='real', cheating_level=.0, numpy_rng=None, theano_rng=None):
        ...
        self.whx = theano.shared(value=self.numpy_rng.uniform( low=-0.01, high=0.01, size=(self.numhid, self.numvis)).astype(theano.config.floatX), name='whx')
        self.wxh = theano.shared(value=self.numpy_rng.uniform( low=-0.01, high=0.01, size=(self.numvis, self.numhid)).astype(theano.config.floatX), name='wxh')
        ...
        def step(x_gt_t, x_tm1, hids_tm1):
            pre_hids_t = T.dot(hids_tm1, self.whh) + T.dot(self.cheating_level*x_gt_t+(1.-self.cheating_level)*x_tm1, self.wxh)
